...
function writeJsDL (output, filesObj) {
  const hashesArray = Object.keys(filesObj).map((files) => filesObj[files].hash)// -> array of hashes[]
  const magnetsArray = Object.keys(filesObj).map((files) => filesObj[files].magnet) // ->array of magnets
